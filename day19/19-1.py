...
class Ocean:
    ...
    def try_to_add_scanner_points(self, scanner_points) -> bool:
        ...
        common_beacons = self.find_beacon_with_enough_common_neighbors(
        ...
        orientation = self.find_orientation(common_beacons)

        # Correct orient of unknown neighbors
        unknown_neighbors = self.new_neighbors(common_beacons, orientation)
    ...
    def new_neighbors(self, common_beacons, orientation):
        ...
        unknown_neighbors = [orient(n, orientation) for n in unknown_neighbors]
    ...
    def find_orientation(self, common_beacons):
        known_beacon_entry, new_beacon_entry, common_neighbors = common_beacons
    ...
    def find_beacon_with_enough_common_neighbors(self, known_beacon_neighbors, new_beacon_neighbors):
        '''
...
def main():
    ...
    while len(next_scanners) > 0 and len(next_scanners) != len(this_scanners):
        ...
        for scanner in this_scanners:
            if ocean.try_to_add_scanner_points(scanner):
                continue
